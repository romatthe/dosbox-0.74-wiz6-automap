...
extern SDL_Window *AutoMapWindow;
extern Uint32 AutoMapWindowID;
extern SDL_GLContext AutoMapContext;
...
static void PauseDOSBox(bool pressed) {
	...
	while (paused) {
		...
		switch (event.type) {
			...
			case SDL_QUIT: MessageBoxA(0,"q","q",0); throw(0); break;
#if SDL_VERSION_ATLEAST(2,0,0)
			case SDL_WINDOWEVENT:
				if (AutoMapWindowID==event.window.windowID) OnAutoMapWindowEvent(event); 
				if (SDL_GetWindowID(sdl.window)==event.window.windowID){
					OnMainWindowEvent(event);
...
//	    || (fullscreen && ((width != currWidth) || (height != currHeight)))
		...
		SDL_SysWMinfo mwInfo;
		SDL_VERSION(&mwInfo.version);
		SDL_GetWindowWMInfo(sdl.window, &mwInfo);
		AutoMapOnMainWindowCreate(mwInfo.info.win.window);
...
void GFX_Events() {
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_WINDOWEVENT:
			if (AutoMapWindowID==event.window.windowID) OnAutoMapWindowEvent(event);
			if (SDL_GetWindowID(sdl.window)!=event.window.windowID) break;
			OnMainWindowEvent(event);
...
//					}
					...
					while (paused) {
						...
						switch (ev.type) {
						case SDL_QUIT: MessageBoxA(0,"q","q",0);throw(0); break; // a bit redundant at linux at least as the active events gets before the quit event.
...
//					}
		...
		case SDL_MOUSEMOTION:
			if (SDL_GetWindowID(sdl.window)==event.motion.windowID){
				HandleMouseMotion(&event.motion);
				OnAutomapMouseMotionInMainWindow(&event.motion);
			}
			if (AutoMapWindowID==event.motion.windowID)
				OnAutomapMouseMotion(&event.motion);
			break;
		case SDL_MOUSEBUTTONDOWN:
		case SDL_MOUSEBUTTONUP:
			if (SDL_GetWindowID(sdl.window)==event.button.windowID) HandleMouseButton(&event.button);
			if (AutoMapWindowID==event.button.windowID) OnAutomapMouseButton(&event.button);
			break;
		case SDL_MOUSEWHEEL:
			if (SDL_GetWindowID(sdl.window)==event.button.windowID) HandleMouseWheel(&event.wheel);
			break;
#endif
#if !SDL_VERSION_ATLEAST(2,0,0)
...
//GFX_HandleVideoResize(event.resize.w, event.resize.h);
			break;
#endif
		case SDL_QUIT:MessageBoxA(0,"q","q",0);
			throw(0);
			break;
#if !SDL_VERSION_ATLEAST(2,0,0)
...
//		UI_Init();
		...
		Section_prop * am_sec=static_cast<Section_prop *>(control->GetSection("Automap"));
		am_show_automap = am_sec->Get_bool("enable");
		am_height = am_sec->Get_int("height");
		am_width = am_sec->Get_int("width");
		GFX_ObtainDisplayDimensions();
		AutoMapWindow = SDL_CreateWindow("undetected - Automap", 14, SDL_WINDOWPOS_CENTERED,
    	am_width, am_height, SDL_WINDOW_OPENGL | ((am_show_automap) ? SDL_WINDOW_SHOWN : SDL_WINDOW_HIDDEN));
		AutoMapWindowID = SDL_GetWindowID(AutoMapWindow);
		AutoMapContext = SDL_GL_CreateContext(AutoMapWindow);
		AutoMapCreate();
